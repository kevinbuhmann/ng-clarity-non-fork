...
import { Story } from './helpers/story-list.helpers';
import { handleUnusedScreenshots } from './helpers/unused-screenshots.helpers';
...
for (const { storyId, component } of stories) {
  ...
  test(screenshotPath, async ({ page }) => {
    await page.goto(`http://localhost:8080/iframe.html?id=${storyId}&viewMode=story`);
